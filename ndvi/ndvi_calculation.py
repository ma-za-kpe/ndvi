...
# def chart(polygon: List[Coord], width: int, date_start: str, date_end: str) -> List[Dict[str, Union[str, float]]]:
...
#     coord_lng = [coord['lng'] for coord in polygon]
#     coord_lat = [coord['lat'] for coord in polygon]
    
#     # Calculate the bounding box
#     max_lng, min_lng = max(coord_lng), min(coord_lng)
#     max_lat, min_lat = max(coord_lat), min(coord_lat)
    
#     # Calculate image dimensions based on the width and aspect ratio of the polygon
#     delta_lng, delta_lat = max_lng - min_lng, max_lat - min_lat
#     dimension_lng = round(width)
#     dimension_lat = round((width * delta_lat) / delta_lng)
    
#     # Calculate the centroid of the polygon
#     centroid_lng = sum(coord_lng) / len(coord_lng)
#     centroid_lat = sum(coord_lat) / len(coord_lat)
...
#     cloudlessNDVI = collection.map(calculate_ndvi)
    
#     print(f"cloudlessNDVI: {cloudlessNDVI}")
...
#     #     return {'date': image.date().format('YYYY-MM-dd'), 'ndvi': value.get('NDVI')}
...
#     timeseries = cloudlessNDVI.map(reduce_image)

#     # Get the NDVI values as a list
#     ndviData = timeseries.getInfo()['features']
#     print(f"ndviData: {ndviData}")
...
def chart(polygon: List[Coord], width: int, date_start: str, date_end: str) -> None:
    ...
    # Reduce the ImageCollection to a time series for the region of interest
    timeseries = collection.map(reduce_image)
